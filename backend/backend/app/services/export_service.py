...
async def export_event_pdf(
    ...
    story.append(Paragraph(
        f"<b>Datum:</b> {event.created_at.strftime('%d.%m.%Y')}",
        styles['Normal']
    ))
    story.append(Paragraph(
        f"<b>Anzahl Einsätze:</b> {len(incidents)}",
        styles['Normal']
    ))
    story.append(Paragraph(
        f"<b>Trainingsmodus:</b> {'Ja' if event.training_flag else 'Nein'}",
        styles['Normal']
    ))
    story.append(Spacer(1, 24))
    story.append(Paragraph(
        f"<b>Exportiert von:</b> {user.username}",
        styles['Normal']
    ))
    story.append(Paragraph(
        f"<b>Exportiert am:</b> {datetime.utcnow().strftime('%d.%m.%Y %H:%M')} UTC",
        styles['Normal']
    ))
    ...
    for idx, incident in enumerate(incidents, 1):
        ...
        if incident.location_address:
            story.append(Paragraph(
                f"Adresse: {incident.location_address}",
                styles['Normal']
            ))
        if incident.description:
            story.append(Paragraph(
                f"Beschreibung: {incident.description}",
                styles['Normal']
            ))
        story.append(Spacer(1, 12))
        ...
        if transitions:
            ...
            story.append(Paragraph(
                f"Erstellt: {incident.created_at.strftime('%d.%m.%Y %H:%M')}",
                styles['Normal']
            ))
        story.append(Spacer(1, 12))
            ...
            for assignment in assignments:
                ...
                if assignment.resource_type == "personnel":
                    res = await db.execute(
                        select(Personnel).where(Personnel.id == assignment.resource_id)
                    )
                    ...
                    res = await db.execute(
                        select(Vehicle).where(Vehicle.id == assignment.resource_id)
                    )
                    ...
                    res = await db.execute(
                        select(Material).where(Material.id == assignment.resource_id)
                    )
        ...
        reko_result = await db.execute(
            ...
            .where(RekoReport.is_draft == False)
        ...
        if reko:
            ...
            if reko.is_relevant is not None:
                story.append(Paragraph(
                    f"Relevanz: {'Relevant' if reko.is_relevant else 'Nicht relevant'}",
                    styles['Normal']
                ))
        ...
        if audit_entries:
            for audit, audit_user in audit_entries:
                ...
                story.append(Paragraph(
                    f"{audit.timestamp.strftime('%d.%m.%Y %H:%M')} - "
                    f"{username}: {audit.action_type}",
                    styles['Normal']
                ))
        else:
...
async def export_event_excel(
    ...
    for row_num, (idx, incident) in enumerate(enumerate(incidents, 1), 2):
        ...
        ws_incidents.cell(
            row=row_num,
            column=7,
            value=incident.created_at.strftime('%d.%m.%Y %H:%M')
        )
        ws_incidents.cell(
            row=row_num,
            column=8,
    ...
    for idx, incident in enumerate(incidents, 1):
        ...
        for assignment in assignments:
            ...
            ws_details.cell(
                row=current_row,
                column=1,
                value=f"{assignment.resource_type.capitalize()}: {resource_name}"
            )
            current_row += 1
