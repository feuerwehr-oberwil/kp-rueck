...
def with_location_display(incident, home_city: str) -> schemas.IncidentResponse:
...
async def incident_with_display(db: AsyncSession, incident) -> schemas.IncidentResponse:
    return with_location_display(incident, await get_home_city(db))


async def incidents_with_display(db: AsyncSession, incidents) -> list[schemas.IncidentResponse]:
