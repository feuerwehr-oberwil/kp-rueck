"""Tests for the Kostenpflicht export endpoint (plan 25, §7).

``GET /api/exports/events/{event_id}/kostenpflicht.xlsx`` — editor-only,
...
class TestKostenpflichtAuth:
    @pytest.mark.asyncio
    async def test_unauthenticated_gets_401(self, client: AsyncClient, test_event: Event):
        response = await client.get(f"/api/exports/events/{test_event.id}/kostenpflicht.xlsx")
    ...
    async def test_viewer_gets_403(self, viewer_client: AsyncClient, test_event: Event):
        ...
        response = await viewer_client.get(f"/api/exports/events/{test_event.id}/kostenpflicht.xlsx")
    ...
    def test_rate_limited_like_its_neighbours(self):
        ...
        limits = limiter._route_limits["app.api.exports.export_event_kostenpflicht"]
...
class TestKostenpflichtSuccess:
    @pytest.mark.asyncio
    async def test_returns_xlsx_with_correct_headers(
        self, editor_client: AsyncClient, test_event: Event, test_incident: Incident
    ):
        response = await editor_client.get(f"/api/exports/events/{test_event.id}/kostenpflicht.xlsx")
        assert response.status_code == 200
        assert response.headers["content-type"] == XLSX_MEDIA_TYPE

        disposition = response.headers.get("content-disposition", "")
        assert "attachment" in disposition
        assert "kostenpflicht-test-event-" in disposition
    ...
    async def test_schadenplatz_without_a_rapport_still_gets_a_row(
        ...
        sheet = openpyxl.load_workbook(io.BytesIO(response.content))["Kostenpflicht"]
    ...
    async def test_a_filed_rapport_reaches_the_sheet(
        ...
        assert row["Schadensart"] == "Sturmschaden"
    ...
    async def test_unknown_event_404(self, editor_client: AsyncClient):
        response = await editor_client.get(f"/api/exports/events/{uuid4()}/kostenpflicht.xlsx")
