...
| **`/setup` asks for an «Einrichtungscode».** | Correct on an internet-facing board (`DOMAIN` set, Railway, or an `https://` CORS origin) that has no accounts yet: being the first visitor is no proof of anything there, so the claim needs a one-time code the backend printed at startup. | `docker compose logs backend \| grep -A1 Einrichtungscode` (Railway: the backend's deploy log). Case, spaces and dashes do not matter. A restart prints a new one; the old one stops working. Set `SETUP_TOKEN` to choose your own (it is then not printed), `SETUP_TOKEN_REQUIRED=false` to go back to first-visit-claims – or `ADMIN_SEED_PASSWORD` to skip `/setup` altogether. The banner is logged at `WARNING`, so a `LOG_LEVEL` of `ERROR` hides it. Five wrong codes lock that address out for five minutes, like the login. |
