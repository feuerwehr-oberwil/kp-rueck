...
| 5 | 17 | [Betrieb, Einrichtung und Release-Zyklus](17-betrieb-und-einrichtung.md) | Skripte + Compose + Doku | Einrichtung als eine Oberfläche statt fünf Handgriffe, Dump vor der Migration, TLS im LAN (bei KP Front vier Funktionen, die sonst still fehlen), `latest` gegen `stable`, Betriebsübersicht, zweiter Schirm. Aus der Durchsicht vor der Veröffentlichung | – |
| 6 | 06 | [i18n: actually translate](06-i18n.md) | Frontend | The machinery is **done** – next-intl, the `NEXT_LOCALE` cookie, deep-partial overlays merged over German, and a picker that only offers locales with real content. `fr.json` and `it.json` are still `{}`. What remains is translation work, not engineering; do it **last** so it absorbs the strings from 09/11/13 in one pass | 09, 11, 13 |
...
| 18 | [Code-Qualität und die Grenze zu KP Front](18-code-qualitaet-und-grenzen.md) | Toten Code erst suchen, dann entfernen (die zentrale API-Fehlermeldung ist stumm, ~4'100 Zeilen unerreichter Code); die kopierte Telemetrie-Prüfung greift nicht; Linting auf null statt einfrieren; der Druckagent braucht ein versioniertes, dokumentiertes Protokoll. Aus der Durchsicht vor der Veröffentlichung |
