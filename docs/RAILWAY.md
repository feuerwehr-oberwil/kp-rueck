...
> in **[`DEPLOYMENT.md`](DEPLOYMENT.md)** run the same code – pick by who runs the server, not
> by which is "real". Railway means somebody else keeps the machine alive; compose on a box in
> the Gerätehaus means the board survives an internet outage. A station that does not want a
> server to look after should take Railway. Either way, start at **[`SETUP.md`](SETUP.md)** for
> the station-level setup that follows deployment.
>
> **They do not run the same artefacts, and that changes how you pin a version.** Compose pulls
> the published GHCR images and `KP_RUECK_TAG` picks which release. Railway builds from a branch
> of your fork (§3.2), so there is no tag to pin – you pin by choosing what you merge into the
> branch Railway watches. Read §8.1 before you ever point it at `main`.
...
- **Secure cookies.** The backend works this out from `CORS_ORIGINS`: yours is `https://…`, so
  the login cookie keeps its `Secure` flag. `AUTH_COOKIE_SECURE` is only an override for the
  case where that inference is wrong, which on Railway it never is. Do not set it – a stale
  `false` copied from a LAN install's `.env` would send login cookies in the clear over TLS.
