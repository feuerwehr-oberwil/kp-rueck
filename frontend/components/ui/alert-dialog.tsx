...
function AlertDialogOverlay({
  ...
  return (
    <AlertDialogPrimitive.Overlay
      ...
      className={cn(
        // NO exit animation — see the note in dialog.tsx. Radix's AlertDialog
        // overlay IS `DialogPrimitive.Overlay` (react-alert-dialog just re-exports
        // it), so it carries the same unconditional inline `pointer-events: auto`
        // and lingered exactly the same way: measured `closed`, 1280x720, inline
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ...
        className={cn(
          // Content leaves with the overlay, for the same two reasons as in
          // dialog.tsx: it is a click sink of its own, and a backdrop that
          // disappears while the panel is still fading reads as a bug.
